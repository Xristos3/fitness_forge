...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      initialRoute: '/',
      routes: {
        '/': (context) => CrossClimbersAdvancedScreen(),
        '/buttkickstandardScreen': (context) => PlankAdvancedScreen(),
      },
    );
  }
}
...
class _CountdownScreenState extends State<CrossClimbersAdvancedScreen> {
  int _seconds = 40;
  Timer? _timer;
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      setState(() {
        if (_seconds > 0) {
          ...
          _timer!.cancel();
          Navigator.push(context, MaterialPageRoute(builder: (context) => PlankAdvancedScreen()));
        }
      });
    });
  }
