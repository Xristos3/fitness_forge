...
import 'package:fitness_forge/ui/screen/guest_squats_standard.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      initialRoute: '/',
      routes: {
        '/': (context) => GuestSidetoSideStandardScreen(),
        '/buttkickstandardScreen': (context) => GuestSquatsStandardScreen(),
      },
    );
  }
}
...
class _CountdownScreenState extends State<GuestSidetoSideStandardScreen> {
  int _seconds = 40;
  Timer? _timer;
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      setState(() {
        if (_seconds > 0) {
          ...
          _timer!.cancel();
          Navigator.push(context, MaterialPageRoute(builder: (context) => GuestSquatsStandardScreen()));
        }
      });
    });
  }
