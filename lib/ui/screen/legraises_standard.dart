...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      initialRoute: '/',
      routes: {
        '/': (context) => LegraisesStandardScreen(),
        '/buttkickstandardScreen': (context) => MountainClimbersStandardScreen(),
      },
    );
  }
}
...
class _CountdownScreenState extends State<LegraisesStandardScreen> {
  int _seconds = 40;
  Timer? _timer;
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      setState(() {
        if (_seconds > 0) {
          ...
          _timer!.cancel();
          Navigator.push(context, MaterialPageRoute(builder: (context) => MountainClimbersStandardScreen()));
        }
      });
    });
  }
